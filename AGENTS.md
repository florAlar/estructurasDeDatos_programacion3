...
2. Lee `progress/tasks.json` y elige **una** tarea con estado `pending`. No
...
|------------------------------|-----------------------------------------------------------|---------------|
| `progress/tasks.json`          | Lista de tareas con estado (pending / in_progress / done) | Siempre, al empezar |
...
| `.copilot/agents/`            | Definiciones de subagentes (líder, implementador, revisor) | Si orquestas trabajo |
| `src/`                       | Código de la aplicación                                   | Para implementar |
| `tests/`                     | Tests automáticos                                         | Para verificar |

## 3. Reglas duras (no negociables)

- **Una sola task a la vez.** No mezcles cambios de varias tareas en la misma sesión.
- **No declares una tarea `done` si el proyecto no compila**
- **Documenta lo que haces** en `progress/current.md` mientras trabajas, no al final.
- **Deja el repositorio limpio** antes de cerrar la sesión (ver §5).
- **Si no sabes algo, busca en `docs/`** antes de inventarlo o pregunta al usuario.

## 4. Cómo elegir una tarea

```
1. Abre progress/tasks.json
...
1. Verifica build sin errores.
2. Si la tarea está acabada: marca `status: "done"` en `progress/tasks.json`.
3. Mueve el resumen de `progress/current.md` al final de `progress/history.md`.
4. Vacía `progress/current.md` dejando solo la plantilla.

## 6. Si te bloqueas

- Relee el plan, o pregunta al usuario.

...
2. Lee `progress/tasks.json` y `progress/current.md`.
3. Verifica build. Si falla, paras y reportas.
...
2. Si requiere investigación previa → lanza **2-3** subagentes `explorer`
   en paralelo (cada uno con una pregunta concreta y acotada).
3. Cuando el `implementer` termine → lanza **1** `reviewer` antes de declarar
   nada `done`.
...
referencias del tipo: "resultado en `progress/explorer/explore_<tema>.md`".
...
> `progress/implementer/impl_<feature>.md` (implementer) y `progress/reviewer/review_<feature>.md`
> (reviewer). Tú, como líder, nunca verás su contenido en chat — solo una
> referencia del tipo `done -> progress/implementer/comoimpl_<feature>.md`. Para reproducirlo
